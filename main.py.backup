...
Version: 5.0 (Added Analytics Integration)
...
async def initialize_services(config: SystemConfig) -> Dict[str, Any]:
    ...
    try:
        ...
        # Note: UBECDataSynchronizer only takes db_manager
    ...
    # 4. Initialize Analytics Service (NEW)
    if services.get('database'):
        try:
            # Try multiple import locations with proper fallbacks
            analytics = None
    ...
    if services.get('database'):
        try:
            # Try to import and instantiate evaluator directly
            evaluator = None
            ...
            try:
                ...
                # UBECHolonicEvaluator takes config_path and db_connection
                evaluator = UBECHolonicEvaluator(
                    config_path=None,  # Will use defaults
                    db_connection=None  # Will create its own connection
                )
            except ImportError:
                try:
...
async def run_sync(services: Dict[str, Any], asset_code: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if asset_code:
            # Sync specific asset
            result = await synchronizer.sync_asset_data(asset_code)
        else:
            # Sync all assets
            results = await asyncio.gather(
                synchronizer.sync_asset_data('UBEC'),
                synchronizer.sync_asset_data('UBECrc'),
                synchronizer.sync_asset_data('UBECgpi'),
                synchronizer.sync_asset_data('UBECtt'),
...
async def run_analytics(services: Dict[str, Any], analysis_type: str = 'summary') -> Dict[str, Any]:
    ...
    try:
        ...
        if analysis_type == 'summary':
            ...
            # Get token distribution for all assets
            results = await asyncio.gather(
            ...
            # Get holder analysis for all assets
            results = await asyncio.gather(
...
def main() -> int:
    ...
    logger.info(f"Version: 5.0 (Added Analytics Integration)")
