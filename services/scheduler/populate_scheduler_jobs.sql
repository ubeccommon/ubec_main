-- ============================================================================
-- UBEC Protocol Scheduler Jobs - Default Configuration
-- ============================================================================
--
...
TRUNCATE TABLE ubec_main.scheduler_jobs RESTART IDENTITY CASCADE;
...
-- ============================================================================
-- Frequency: Daily at 2 AM (86400 seconds = 24 hours)
...
INSERT INTO ubec_main.scheduler_jobs (
    ...
    '86400',  -- 24 hours
    (DATE_TRUNC('day', NOW()) + INTERVAL '1 day' + INTERVAL '2 hours'),  -- Tomorrow 2 AM
...
ORDER BY schedule_interval::integer;
...
-----------------------+----------+---------------------------+---------+---------------------------
 blockchain_sync       | 300s     | 2025-11-05 10:30:00       | t       | sync.sync_incremental
 protocol_health_check | 600s     | 2025-11-05 10:33:00       | t       | air_protocol.refresh_cache
 analytics_update      | 900s     | 2025-11-05 10:32:00       | t       | analytics.update_analytics
 holonic_evaluation    | 1800s    | 2025-11-05 10:35:00       | t       | holonic.evaluate_all_accounts
 bioregion_analysis    | 3600s    | 2025-11-05 10:45:00       | t       | bioregion_manager.update_bioregions
 report_generation     | 21600s   | 2025-11-05 10:40:00       | t       | visualizer.generate_html_report
...
-- ============================================================================
--
-- 1. Jobs are staggered to avoid simultaneous execution
-- 2. Intervals can be modified via UPDATE statements
-- 3. Jobs can be enabled/disabled via UPDATE statements
-- 4. New jobs can be added via INSERT statements
--
-- Example: Disable a job
-- UPDATE ubec_main.scheduler_jobs SET enabled = false WHERE job_name = 'report_generation';
--
-- Example: Change interval to 10 minutes
-- UPDATE ubec_main.scheduler_jobs SET schedule_interval = '600' WHERE job_name = 'analytics_update';
--
-- ============================================================================
