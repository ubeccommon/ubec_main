    ...
    def _save_as_markdown(self, filename: str):
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            for table_info in summary['tables_by_size'][:10]:
                f.write(f"| {table_info['table']} | {table_info['rows']:,} | {table_info['size']} |\n")
            f.write("\n---\n\n")
            ...
            for element in ['core', 'air', 'water', 'earth', 'fire']:
                ...
                for table_name in sorted(tables):
                    ...
                    f.write(f"**Statistics:** {stats['row_count']:,} rows | ")
                    f.write(f"Table: {stats['table_size']} | ")
                    f.write(f"Indexes: {stats['index_size']} | ")
                    f.write(f"Total: {stats['total_size']}\n\n")
