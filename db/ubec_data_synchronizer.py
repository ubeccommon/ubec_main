...
class UBECDataSynchronizer:
    ...
    def __init__(self, config_path="../config/settings.py", db_manager=None):
        ...
        logger.info(f"Initializing UBEC Data Synchronizer (Async) with config: {config_path}")
        ...
        logger.info("✓ UBEC Data Synchronizer (Async) initialized successfully")
    ...
    def _initialize_stellar_connection(self):
        ...
        try:
            ...
            self.server = ServerAsync(horizon_url=self.settings.HORIZON_URL)
    ...
    async def _check_rate_limit(self, buffer: int = 10) -> bool:
        """
    ...
    async def _handle_rate_limit_error(self, response) -> int:
        """
    ...
    async def _save_account(self, account_id: str, element: Optional[str] = None, 
    ...
    async def _ensure_account_exists(self, account_id: str, element: Optional[str] = None, 
        ...
        try:
            ...
            # Create account
            account_id_result = await self._save_account(account_id, element, token_code)
    ...
    async def _save_balance(self, account_id: str, token_code: str, element: str, 
        ...
        Args:
            ...
            distribution_category: Optional category
    ...
    # ========================================================================
    # TRANSACTION FETCHING FROM STELLAR - ASYNC VERSION
    # ========================================================================
    
    async def fetch_account_history(self, account: str, days: int = 30, 
                                    max_retries: int = 3) -> List[Dict[str, Any]]:
        """
        Fetch transaction history for an account from Stellar blockchain (async).
        ...
        try:
            while True:
                try:
                    ...
                    op_response = await op_endpoint.call()
                    ...
                    for op in records:
                        try:
                            ...
                            tx_data = await self._process_operation(op)
                            if tx_data:
                    ...
                    if hasattr(e, 'status_code') and e.status_code == 429:
                        ...
                        wait_time = await self._handle_rate_limit_error(e.response if hasattr(e, 'response') else None)
    ...
    async def _process_operation(self, op: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Process a single operation and extract transaction data.
        
        Args:
            op: Operation data from Stellar
            
        Returns:
            dict: Transaction data or None if not relevant
        """
        op_type = op.get('type', '')
    ...
    async def _store_transactions(self, account_id: str, transactions: List[Dict[str, Any]]) -> int:
        """
        ...
        try:
            ...
            if not await self._ensure_account_exists(account_id):
                logger.error(f"✗ Cannot store transactions - account {account_id[:8]}... doesn't exist")
                return 0
            
            # Extract all unique source accounts
            source_accounts = set()
    ...
    async def _store_single_transaction(self, tx: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            await self._store_transaction_record(tx, element, asset_code)
            
            # Store operation record
            await self._store_operation_record(tx, element, asset_code)
            
            return True
            
        except Exception as e:
            logger.error(f"Error storing single transaction: {e}")
            return False
    
    async def _store_transaction_record(self, tx: Dict[str, Any], element: str, asset_code: str):
        """Store transaction record."""
        tx_query = """
    ...
    async def _store_operation_record(self, tx: Dict[str, Any], element: str, asset_code: str):
        """Store operation record."""
        op_query = """
    ...
    # ========================================================================
    # HIGH-LEVEL SYNC OPERATIONS - ASYNC VERSION
    # ========================================================================
    
    async def sync_account_transactions(self, account_id: str, days_back: int = 30) -> int:
        """
        Fetch and store transaction history for an account (async).
        ...
        try:
            ...
            transactions = await self.fetch_account_history(account_id, days_back)
            
            # Store in database
            count = await self._store_transactions(account_id, transactions)
    ...
    async def discover_new_holders(self, days_back: int = 30, batch_size: int = 10, 
                                  batch_delay: int = 3) -> int:
        """
        Discover new token holders from recent transactions (async).
            ...
            for i in range(0, len(new_accounts), batch_size):
                ...
                for account in batch:
                    ...
                    try:
                        ...
                        if not await self._ensure_account_exists(account, element, self.ubec_code):
                            logger.error(f"✗ Failed to create account for {account[:8]}..., skipping")
                            continue
                        
                        # Check balance on blockchain
                        if self.server:
                            success = await self._fetch_and_save_account_balance(account, element)
                            if success:
                                count += 1
                                logger.debug(f"✓ New holder: {account[:8]}...")
                            
                            await asyncio.sleep(0.2)
                        
                    except Exception as e:
                        if hasattr(e, 'status_code') and e.status_code == 429:
                            wait_time = await self._handle_rate_limit_error(
    ...
    async def _fetch_and_save_account_balance(self, account: str, element: str) -> bool:
        """
        Fetch and save account balance from blockchain.
        ...
        try:
            ...
            stellar_account = await self.server.accounts().account_id(account).call()
            ...
            for balance_data in stellar_account.get('balances', []):
                if (balance_data.get('asset_code') == self.ubec_code and
                    ...
                    if bal_amount > 0:
                        success = await self._save_balance(
                            account,
                            self.ubec_code,
                            element,
                            bal_amount
            ...
            logger.error(f"Error fetching/saving balance for {account[:8]}...: {e}")
            return False
    
    async def find_all_holders_from_network(self, batch_size: int = 25, max_accounts: int = 200, 
                                           batch_delay: int = 10) -> int:
        """
        Find all token holders directly from Stellar network (async).
        ...
        try:
            while count < max_accounts:
                try:
                    ...
                    # Query Stellar for accounts with trustlines
                    ...
                    # Process batch
                    batch_results = await self._process_account_batch(records, element)
                    count += batch_results
    ...
    async def _process_account_batch(self, records: List[Dict], element: str) -> int:
        """
        Process a batch of account records.
        
        Args:
            records: Account records from Stellar
            element: Element type
            
        Returns:
            int: Number of accounts processed successfully
        """
        count = 0
        ...
        for account in records:
            ...
            if ubec_balance > 0:
                try:
                    ...
                    account_db_id = await self._save_account(account_id, element, self.ubec_code)
    ...
    async def sync_account_data(self, asset_code: str = 'UBEC', issuer: Optional[str] = None, 
                               limit: int = 200) -> Dict[str, Any]:
        """
        Synchronize account data from Stellar network (async).
        ...
        try:
            ...
            accounts_found = await self.find_all_holders_from_network(
                batch_size=25,
                max_accounts=limit,
                batch_delay=10
            )
            
            # PHASE 2: Check recent transactions
            logger.info(f"\nPhase 2: Discovering holders from recent transactions...")
            additional_found = await self.discover_new_holders(
                days_back=30,
                batch_size=10,
                batch_delay=3
            )
    ...
    async def sync_transaction_data(self, asset_code: str = 'UBEC', issuer: Optional[str] = None, 
                                   days_back: int = 7) -> Dict[str, Any]:
        """
        Synchronize transaction data from Stellar network (async).
        ...
        try:
            ...
            for i, holder in enumerate(holders):
                ...
                try:
                    ...
                    txs = await self.sync_account_transactions(account_id, days_back)
    ...
    async def sync_balance_data(self, asset_code: str = 'UBEC', issuer: Optional[str] = None) -> Dict[str, Any]:
        """
        Synchronize balance data from Stellar network (async).
        ...
        try:
            ...
            if not accounts:
                ...
                discovery = await self.sync_account_data(asset_code, issuer, 100)
                accounts = self.db.execute_query(query)
            ...
            for i, account in enumerate(accounts[:50]):
                ...
                try:
                    ...
                    if self.server:
                        stellar_account = await self.server.accounts().account_id(account_id).call()
                        ...
                        for balance_data in stellar_account.get('balances', []):
                            if (balance_data.get('asset_code') == asset_code and
                                ...
                                success = await self._save_balance(account_id, asset_code, element, bal_amount)
...
# ============================================================================
# ASYNC WRAPPER FOR SYNC METHODS (If needed for compatibility)
# ============================================================================

def run_async(coro):
    """
    Helper to run async coroutines in sync context if needed.
...
# ============================================================================
# MAIN - For testing (Testing block is allowed)
# ============================================================================
...
async def main():
    ...
    print("\n" + "=" * 70)
    print("TESTING UBEC DATA SYNCHRONIZER (Async Version)")
    print("=" * 70)
    
    async with UBECDataSynchronizer() as sync:
        try:
            print("\n1. Testing account sync...")
            result = await sync.sync_account_data(asset_code='UBEC', limit=50)
            print(f"   Result: {result}")
            
            print("\n2. Testing balance sync...")
            result = await sync.sync_balance_data(asset_code='UBEC')
            print(f"   Result: {result}")
            
            print("\n3. Testing transaction sync...")
            result = await sync.sync_transaction_data(asset_code='UBEC', days_back=7)
            print(f"   Result: {result}")
