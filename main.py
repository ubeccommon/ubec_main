...
Integrated Services:
    ...
    - Distribution Evaluator (Tokenomics Compliance)
...
def register_core_services():
    ...
    # ========================================================================
    # ORDER BOOK SERVICE (UPDATED: Now calls initialize())
    # ========================================================================
    
    async def create_orderbook(registry: ServiceRegistry):
        """
        Create order book service with proper initialization.
        
        CRITICAL: Order book service uses api_dependent_health() pattern.
        Must call initialize() after creation for proper health checks.
        """
        ...
        # CRITICAL: Initialize the service for proper health check support
    ...
    # ========================================================================
    # PROTOCOL SERVICES (Elements) - UPDATED: Now call initialize()
    # ========================================================================
    ...
    def create_protocol_factory(element: str, module_path: str):
        ...
        CRITICAL: All protocol services use api_dependent_health() pattern.
        Must call initialize() after creation for proper health checks.
        """
        async def factory(registry: ServiceRegistry):
            ...
            element_config = {
                'asset_code': getattr(
                    config, 
                    f'UBEC{element.upper()}_CODE' if element else 'UBEC_CODE'
                ),
                'issuer': getattr(
                    config, 
                    f'UBEC{element.upper()}_ISSUER' if element else 'UBEC_ISSUER'
                ),
                'element': {
                    '': 'air', 
                    'rc': 'water', 
                ...
                'principle': {
                    ...
                    'gpi': 'mutualism', 
                    'tt': 'regeneration'
                }.get(element, 'diversity')
            }
            
            # Create service (factory may be async)
    ...
    async def create_audit(registry: ServiceRegistry):
        ...
        audit_config = {
            ...
            'stewardship_account': (
                config.ACCOUNTS.get('stewardship', [''])[0] 
                if isinstance(config.ACCOUNTS.get('stewardship'), list) 
                else config.ACCOUNTS.get('stewardship', '')
            ),
            'tokenomics': {
    ...
    # ========================================================================
    # HOLONIC EVALUATOR (Uses database_only_health())
    # ========================================================================
    ...
    async def create_holonic_evaluator(registry: ServiceRegistry):
        ...
        This service uses database_only_health() pattern.
        The factory function automatically calls initialize().
        """
        ...
        # Factory automatically calls initialize()
    ...
    async def create_visualizer(registry: ServiceRegistry):
        ...
        Factory function automatically includes initialization.
        """
        ...
        # Factory automatically calls initialize() and verifies ready status
    ...
    # ========================================================================
    # DISTRIBUTION EVALUATOR (Uses service_dependent_health())
    # ========================================================================
    ...
    async def create_dist_evaluator(registry: ServiceRegistry):
        ...
        This service uses service_dependent_health() pattern.
        It depends on distribution_service and audit_service.
        """
        ...
        # Factory returns initialized service
...
async def run_health_check() -> Dict[str, Any]:
    ...
    All services now use standardized ServiceHealthCheck utility:
    - api_dependent_health() for protocols and orderbook
    - database_only_health() for holonic evaluator
    - service_dependent_health() for distribution evaluator
    """
    ...
    # Add summary with pattern information
    patterns_used = {
        'api_dependent': ['air', 'water', 'earth', 'fire', 'orderbook'],
        'database_only': ['holonic_evaluator'],
        'service_dependent': ['distribution_evaluator']
    }
    
    health['health_check_patterns'] = patterns_used
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--chart-type', 
        type=str, 
        choices=['radar', 'score_distribution', 'category_distribution', 
    ...
    parser.add_argument(
        ...
        type=int, 
        default=60, 
        help='Time period in minutes for flow analysis'
    )
    parser.add_argument(
        '--threshold-pct', 
        type=float, 
        default=5.0, 
        help='Whale detection threshold'
    )
...
def main() -> int:
    ...
    logger.info("  • api_dependent_health() - 5 services")
    logger.info("  • database_only_health() - 1 service")
    logger.info("  • service_dependent_health() - 1 service")
    logger.info("=" * 70)
