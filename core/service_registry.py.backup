...
class ServiceRegistry:
    ...
    def _resolve_dependencies(self, services: List[str]) -> List[str]:
        ...
        # Calculate in-degrees - FIXED!
        # A service's in-degree is the number of dependencies it has
        # Services with 0 dependencies can be initialized first
        for service in all_services:
            in_degree[service] = len(graph[service])
